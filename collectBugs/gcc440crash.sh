#!/bin/bash
BADCC1=("/path/to/gcc-4.4.0/bin/gcc -O0" "/path/to/gccs/gcc-4.4.0/bin/gcc -O1" "/path/to/gccs/gcc-4.4.0/bin/gcc -O2" "/path/to/gccs/gcc-4.4.0/bin/gcc -O3" "/path/to/gccs/gcc-4.4.0/bin/gcc -Os")
BADCC2=()
BADCC3=()
MODE=""
...
GOODCC=("/path/to/gccs/gcc-4.8.0/bin/gcc -O0")
TIMEOUTCC=20
TIMEOUTEXE=2
TIMEOUTCCOMP=10
CFILE=small.c
CFLAG="-o t"
CLANGFC="clang $MODE -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined"
...
for cc in "${BADCC1[@]}" ; do 
    ...
    if ! grep 'Please submit a full bug report' out2.txt && \
       ! grep 'with preprocessed source if appropriate' out2.txt
    then	
        continue
    fi
    crashflag=1
